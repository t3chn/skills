...
How bd-issue-tracking integrates with update_plan, writing-plans, and other skills for optimal workflow.

## Contents

- [update_plan Integration](#update_plan-integration) - Temporal layering pattern
...
**update_plan** (short-term working memory - this hour):
...
**Key insight**: update_plan = working copy for the current hour. Beads = project journal for the current month.

### The Handoff Pattern

1. **Session start**: Read bead → Create update_plan items for immediate actions
2. **During work**: Mark update_plan items completed as you go
3. **Reach milestone**: Update bead notes with outcomes + context
4. **Session end**: update_plan disappears, bead survives with enriched notes

**After context loss**: update_plan is gone, but bead notes reconstruct what happened.

### Example: update_plan tracks execution, Beads capture meaning

**update_plan (ephemeral execution view):**
```
...
**What's different**:
- update_plan: Task names (what to do)
- Beads: Outcomes and decisions (what was learned, why it matters)

**Don't duplicate**: update_plan tracks execution, Beads captures meaning and context.

### When to Update Each Tool

**Update update_plan** (frequently):
...
**Pattern**: update_plan changes every few minutes. Beads updates every hour or at natural breakpoints.
...
See BOUNDARIES.md for bd vs update_plan decision.
"

# Mark in_progress
bd update oauth-1 --status in_progress

# Create update_plan for today's work
update_plan:
...
# update_plan disappears when session ends
```
...
# Create fresh update_plan from NEXT
update_plan:
...
3. **update_plan**: Current task within the plan
...
2. Use update_plan to track current task within plan
...
- **Complex feature**: bd issue + update_plan
- **Very complex feature**: bd issue + writing-plans + update_plan
...
4. update_plan tracks immediate writing tasks:
   ```
...
**Why this works**: bd preserves context across sessions (document might take days), skill provides writing framework, update_plan tracks current work.
...
2. Work through subtasks in order, using update_plan for each:
   ```
   Current: login-1
   update_plan:
...
**Why this works**: bd tracks dependencies and progress across files, update_plan focuses on current file, skills provide specialized frameworks when needed.
...
| Need | Tool | Why |
|------|------|-----|
| Track today's execution | update_plan | Lightweight, shows current progress |
...
├─ Yes → Use update_plan only
└─ No → Use bd
    ├─ Simple feature → bd issue + update_plan
    └─ Complex feature → bd issue + writing-plans + update_plan

Will conversation history get compacted?
├─ Likely → Use bd (context survives)
└─ Unlikely → update_plan is sufficient

Does work have dependencies or blockers?
├─ Yes → Use bd (tracks relationships)
└─ No → update_plan is sufficient
...
└─ General tracking → bd + update_plan
```

### Integration Anti-Patterns

**Don't**:
- Duplicate update_plan tasks into bd notes (different purposes)
- Create bd issues for single-session linear work (use update_plan)
- Put detailed implementation steps in bd notes (use writing-plans)
- Update bd after every update_plan task (update at milestones)
- Use writing-plans for exploratory work (defeats the purpose)

**Do**:
- Update bd when changing tools or reaching milestones
- Use update_plan as "working copy" of bd's NEXT section
...
- **update_plan**: Minutes to hours (current execution)
