...
2. Create update_plan for immediate work:
   ```
   - [ ] Draft cost-benefit analysis
   - [ ] Ask user about budget constraints
   - [ ] Finalize recommendations
   ```
3. Work on tasks, mark update_plan items completed
...
**Outcome**: update_plan disappears at session end, but bd notes preserve context for next session.

**Key insight**: Notes field captures the "why" and context, update_plan tracks the "doing" right now.
...
```bash
bd create "Fix: payment validation bypasses card expiry check" -t bug -p 0
bd dep add checkout-1 payment-bug-2 --type discovered-from
bd update checkout-1 --status blocked --notes "Blocked by payment-bug-2: security hole in validation"
bd update payment-bug-2 --status in_progress
```
...
2. **Check what's stuck**: `bd blocked`
3. **Check recent progress**: `bd list --status closed`
4. **Read detailed context**: `bd show <id>`
5. **Update status**: `bd update <id> --status in_progress`
6. **Begin work**: Create update_plan from notes field's NEXT section
...
# Now create update_plan based on NEXT section
```
...
3. **Reconstruct update_plan from notes**:
   - COMPLETED section: Done, skip
   - IN PROGRESS section: Current state
   - NEXT section: **This becomes your update_plan list**
