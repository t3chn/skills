...
def _read_gitmodules_url_branch(root: Path) -> tuple[str | None, str | None]:
    ...
    branch_res = _git(
        ["config", "-f", ".gitmodules", "--get", f"submodule.{name}.branch"],
        cwd=root,
        check=False,
    )
...
def cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    codex_skills_registered = (
        _find_submodule_name_by_path(root, str(SUBMODULE_REL)) is not None if is_git_repo else False
    )
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="skillsctl",
        description="Manage repo-scoped Codex skills via submodule+sparse.",
    )
